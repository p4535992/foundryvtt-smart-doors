...
export function onCanvasReady(currentCanvas) {
	if (game.settings.get(settingsKey, "highlightSecretDoors")) {
		...
		const secretDoors = canvas.controls.doors.children.filter((control) => control.wall.door == types.SECRET);
...
export function onUpdateWall(wall, update, options) {
	...
	else console.warn("Smart Doors | Encountered unknown door type " + wall.door + " while highlighting secret doors.");
}
